...
# App sheet (GPS-tracked, first-last name format)
SHEET_APP = "Time Per Round App"
COL_DATE     = 0
COL_EVENT    = 1
COL_NAME     = 2
...
# Scorecard sheet (manually entered, last-first name format)
SHEET_SC     = "Time Per Round Scorecard"
SC_COL_NAME  = 2   # "Participant" — "Last, First" format
SC_COL_GROUP = 3   # booking group
SC_COL_ROUND = 6   # "Round Time" — HH:MM string
...
def flip_last_first(raw):
    ...
    if "," in raw:
        parts = raw.split(",", 1)
        return normalise_name(parts[1].strip() + " " + parts[0].strip())
...
def process_files(xls_paths):
    ...
    for path in xls_paths:
        ...
        # ── App sheet ────────────────────────────────────────────────
        if SHEET_APP in wb.sheet_names():
            sh = wb.sheet_by_name(SHEET_APP)
            file_rows = 0
            for r in range(1, sh.nrows):
                ...
                file_rows += 1
                total_rows += 1
            print(f"  {path.name} [App]: {file_rows:,} valid rows")
        else:
            print(f"  WARNING: '{SHEET_APP}' not found in {path.name}")

        # ── Scorecard sheet ──────────────────────────────────────────
        if SHEET_SC in wb.sheet_names():
            sh = wb.sheet_by_name(SHEET_SC)
            sc_rows = 0
            for r in range(1, sh.nrows):
                row = [sh.cell_value(r, c) for c in range(sh.ncols)]
                name_raw = str(row[SC_COL_NAME]).strip()
                if not name_raw:
                    continue
                if is_nine_hole(row[SC_COL_GROUP]):
                    skipped_nine += 1
                    continue
                mins = parse_mins(row[SC_COL_ROUND])
                if mins is None or mins < MIN_MINS or mins > MAX_MINS:
                    skipped_time += 1
                    continue
                name = flip_last_first(name_raw)
                if name not in players:
                    players[name] = {"times": []}
                players[name]["times"].append(mins)
                sc_rows += 1
                total_rows += 1
            print(f"  {path.name} [Scorecard]: {sc_rows:,} valid rows")
